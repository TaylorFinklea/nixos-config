#!/bin/bash
# Author - https://github.com/masterfink
...
#########################################################
KEY="eKvpQpeUrNAGEQjWKWaHVeGHu447RQwk"
LAT="39.189854"
LON="-94.865131"
...
if $LOW_CALL; then
    ...
    if [ -n "$current" ] && [ -n "$sun" ]; then
        ...
        nowEpoch=$(gdate --date=$(echo $now | cut -d "\"" -f 2) +%s)
        hourEpoch=$(gdate --date=$(echo $hour | cut -d "\"" -f 2) +%s)
        sunsetEpoch=$(gdate --date=$(echo $sunset | cut -d "\"" -f 2) +%s)
        # nowEpoch=$(date --date=${now:1:-1} +"%s")
        # hourEpoch=$(date --date=${hour:1:-1} +"%s")
        # sunsetEpoch=$(date --date=${sunset:1:-1} +"%s")
        current_temp=$(echo "$current" | jq ".data.timelines[0].intervals[0].values.temperature" | cut -d "." -f 1 )
        current_humidity=$(echo "$current" | jq ".data.timelines[0].intervals[0].values.humidity" | cut -d "." -f 1)
        forecast_temp=$(echo "$current" | jq ".data.timelines[0].intervals[12].values.temperature" | cut -d "." -f 1 )
...
#         ;;
# esac
STATUS="$(get_icon "$current_icon") $current_temp$SYMBOL 󱡕$current_humidity $trend$(get_icon "$forecast_icon") $forecast_temp$SYMBOL"
echo $STATUS
